...
- `@maxhub/max-bot-api` — официальная библиотека для создания ботов в мессенджере MAX. Используется для обработки команд пользователей и отправки уведомлений через long polling
- `node-cron` — cron-планировщик для Node.js. Используется для отправки уведомлений о мероприятиях по расписанию (утреннее уведомление в 7:00 МСК и напоминание за 30 минут)
